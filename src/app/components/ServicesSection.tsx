// ServicesSection.tsx
// Services section component for the home page, fetches data from Sanity
...
const fallbackData: ServicesSectionData = {
    ...
    description: 'While we specialize in high-converting long-form content, we\'re masters of all formats—from quick social media hits to comprehensive educational campaigns.',
    creativeDevelopment: {
...
export default async function ServicesSection() {
    // Fetch services section data from Sanity CMS
    const data: ServicesSectionData = await sanityClient.fetch(
        ...
        <section className="py-20 px-4 bg-white relative overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto text-center relative z-10">
                <h2 className="text-4xl md:text-6xl font-black mb-8 text-black leading-tight">
                    <span className="block mb-2">{sectionData.headline}</span>
                    <span className="block text-red-600">{sectionData.subhead}</span>
                </h2>
                ...
                <div className="relative mb-16">
                    ...
                    <div className="relative z-10 grid lg:grid-cols-3 gap-8">
                        ...
                        <div className="group bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 border border-gray-200 hover:border-red-300 transition-all duration-300 hover:shadow-xl relative">
                            ...
                            <h3 className="text-2xl font-bold text-black mb-4 text-center">{sectionData.creativeDevelopment.title}</h3>
                            <ul className="space-y-3 text-left">
                                {sectionData.creativeDevelopment.services.map((service, idx) => (
                        ...
                        <div className="group bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 border border-gray-200 hover:border-red-300 transition-all duration-300 hover:shadow-xl relative">
                            ...
                            <h3 className="text-2xl font-bold text-black mb-4 text-center">{sectionData.production.title}</h3>
                            <ul className="space-y-3 text-left">
                                {sectionData.production.services.map((service, idx) => (
                                    <li key={idx} className="flex items-start space-x-3">
                                        <div className="w-2 h-2 rounded-full bg-red-600 flex-shrink-0 mt-2"></div>
                                        <span className="text-gray-700">{service}</span>
                                    </li>
                        ...
                        <div className="group bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 border border-gray-200 hover:border-red-300 transition-all duration-300 hover:shadow-xl relative">
                            ...
                            <h3 className="text-2xl font-bold text-black mb-4 text-center">{sectionData.postProduction.title}</h3>
                            <ul className="space-y-3 text-left">
                                {sectionData.postProduction.services.map((service, idx) => (
                                    <li key={idx} className="flex items-start space-x-3">
                                        <div className="w-2 h-2 rounded-full bg-red-600 flex-shrink-0 mt-2"></div>
                                        <span className="text-gray-700">{service}</span>
                                    </li>
